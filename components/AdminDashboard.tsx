...
import { 
  ...
  Target, Zap as ZapIcon, Terminal, BarChart4, Trophy
} from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ products, orders }) => {
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'security' | 'audits'>('overview');
  ...
  const renderOverview = () => (
    <div className="space-y-12 animate-in">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        {[
          { label: 'Revenue', value: '₹1,24,500', change: '+12.5%', icon: DollarSign, color: 'emerald' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
         <div className="lg:col-span-2 bg-white/5 p-12 rounded-[56px] border border-white/5 shadow-3xl">
            <div className="flex justify-between items-center mb-12">
               <div>
                 <h4 className="text-2xl font-black italic tracking-tighter text-white">Identity Growth</h4>
                 <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Global distribution across nodes</p>
               </div>
            </div>
             ...
             <tbody className="divide-y divide-white/5">
                {MOCK_USERS.map(u => (
                  <tr key={u.id} className="hover:bg-white/5 transition-colors group">
                     ...
                     <td className="px-10 py-8">
                        {/* Fix: Added Trophy to imports from lucide-react */}
                     ...
                     <td className="px-10 py-8 text-xs font-black text-slate-500 uppercase">{u.status}</td>
                  </tr>
                ))}
             </tbody>
          </table>
  ...
  return (
    <div className="space-y-16 animate-in bg-slate-950 min-h-screen p-12">
       <div className="flex justify-between items-end">
          <div>
            <h1 className="text-8xl font-black italic tracking-tighter leading-none mb-6 text-white uppercase">Ops Center.</h1>
            <p className="text-xl text-slate-500 font-medium italic font-display">Command & Control Interface v4.1.0</p>
          </div>
       ...
       {activeTab === 'security' && <div className="p-20 bg-white/5 rounded-[64px] border border-white/5 text-center text-white"><ShieldAlert size={60} className="mx-auto mb-8 text-rose-500" /><p className="text-xl font-bold italic">Identity matrix locked. Hardware key required.</p></div>}
    </div>
  );
};
